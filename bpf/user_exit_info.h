...
#define UEI_DEFINE(__name)                                                     \
  ...
  const volatile __u32 __name##_dump_len;                                      \
  struct user_exit_info __name SEC(".data")

#define UEI_RECORD(__uei_name, __ei)                                           \
  ({                                                                           \
    bpf_probe_read_kernel_str(__uei_name.reason, sizeof(__uei_name.reason),    \
                              (__ei)->reason);                                 \
    bpf_probe_read_kernel_str(__uei_name.msg, sizeof(__uei_name.msg),          \
                              (__ei)->msg);                                    \
    bpf_probe_read_kernel_str(__uei_name##_dump, __uei_name##_dump_len,        \
                              (__ei)->dump);                                   \
...
#define UEI_SET_SIZE(__skel, __ops_name, __uei_name)                           \
  ({                                                                           \
    __u32 __len =                                                              \
        (__skel)->struct_ops.__ops_name->exit_dump_len ?: UEI_DUMP_DFL_LEN;    \
...
#define UEI_ECODE_RESTART(__ecode)                                             \
